...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // AeroDataBox sometimes returns flights from nearby dates if the exact date has no data
    const firstFlightDate = data[0]?.departure?.scheduledTime?.local || data[0]?.departure?.scheduledTimeLocal;
    if (firstFlightDate) {
      const returnedDate = firstFlightDate.split("T")[0] || firstFlightDate.split(" ")[0];
      if (returnedDate !== flightDate) {
        console.log("[Flight-Info API] Date mismatch! Requested:", flightDate, "Got:", returnedDate);
        return NextResponse.json(
